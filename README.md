...
1- Login to Deta:
You'll need to provide an access token (get it from your admin console then copy/paste):
```
space login
```
P.S. You can store your Deta project key (e.g. `DETA_PROJECT_KEY`) at `Replit -> Tools -> Secrets` then use it in your code:
```python
import os
...
space push
```
Then open the preview link you see in the console.
